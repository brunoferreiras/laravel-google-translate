...
class TranslateFilesCommand extends Command
{
    ...
    public function handle()
    {
        ...
        if (! $this->json) {
            ...
            $this->excluded_files = ['auth,pagination,validation,passwords'];
            ...
            if (is_array($to_be_translateds)) {
                foreach ($to_be_translateds as $key => $to_be_translated) {
                    if (is_array($to_be_translated)) {
                        foreach ($to_be_translated as $sub_key => $item) {
                            if (isset($already_translateds[$sub_key][$key]) && $already_translateds[$sub_key][$key] != '' && ! $this->force) {
                                $new_lang[$key][$sub_key] = $already_translateds[$sub_key][$key];
        
                                if ($this->verbose) {
                                    $this->line('Exists Skipping -> '.$item.' : '.$new_lang[$key][$sub_key]);
                                }
        
                                continue;
                            }
                            $new_lang[$key][$sub_key] = $this->translate_attribute($item, $locale);
                        }
                    } else {
